...
import modal, sys, time, json

app = modal.App("leghemoglobin-absrel")
...
if __name__ == "__main__":
    ...
    with app.run():
        t0 = time.time()
        res = run_pipeline.remote(cds_fasta, tree_newick)
        print(f"[driver] remote call finished in {time.time()-t0:.1f}s", flush=True)
    ...
    print("HYPHY STDOUT TAIL:", res.get("hyphy_stdout", "")[-2000:])
