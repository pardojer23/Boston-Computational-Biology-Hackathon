...
a rule invoking ``pipeline/steps.py``.
    ...
    snakemake -s workflow/Snakefile --cores 8 --use-conda results/sequence_module/esm2_cosine_distance_matrix.csv
...
TARGET = "results/sequence_module/esm2_cosine_distance_matrix.csv"

print(__doc__.replace("{target}", TARGET), file=sys.stderr)
raise SystemExit(
    "This entry point no longer runs anything. Use:\n"
    f"    snakemake -s workflow/Snakefile --cores 8 --use-conda results/sequence_module/esm2_cosine_distance_matrix.csv\n"
)
